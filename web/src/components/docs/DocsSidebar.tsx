...
export function DocsSidebar({ owner, repo, modules }: DocsSidebarProps) {
  ...
  return (
    <aside className="w-64 border-r bg-muted/30 shrink-0 hidden md:block flex flex-col">
      <ScrollArea className="flex-1 h-[calc(100vh-3.5rem)]">
        <div className="p-4">
          ...
          <div className="mb-4">
            <Link href={basePath} className="text-xs text-muted-foreground hover:text-foreground transition-colors">
              {owner}/{repo}
            </Link>
          </div>
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <div key={item.href}>
                  ...
                  {/* Show module sub-items when on modules page */}
                  {item.href === "/modules" &&
                    isActive &&
                    modules &&
                    modules.length > 0 && (
